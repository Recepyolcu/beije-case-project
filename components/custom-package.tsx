...
export default function CustomPackage() {
    ...
    useEffect(() => {
        setTotalPrice(total());
    }, [pedler, günlükler, tamponlar])

    

    return (
        
        <div className="max-w-[1150px] pt-[152px] pb-[96px] flex items-baseline gap-32">
            <div className="flex flex-col gap-20">
                ...
                <div className={tab == '0' ? "flex flex-col items-start" : tab == '1' ? "flex flex-col items-center" : "flex flex-col items-end"}>
                    <ul className="w-full flex justify-between text-sm font-semibold">
                        <li className="w-full p-3 text-center"><button id="0" className={tab == '0' ? 'text-black' : 'text-[rgba(0,0,0,0.6)]'} onClick={handleChangeTab}>beije Ped</button></li>
                        <li className="w-full p-3 text-center"><button id="1" className={tab == '1' ? 'text-black' : 'text-[rgba(0,0,0,0.6)]'} onClick={handleChangeTab}>beije Günlük Ped</button></li>
                        <li className="w-full p-3 text-center"><button id="2" className={tab == '2' ? 'text-black' : 'text-[rgba(0,0,0,0.6)]'} onClick={handleChangeTab}>beije Tampon</button></li>
                    </ul>
            ...
            <div className="min-w-[460px] flex flex-col items-center gap-7 bg-white rounded-2xl p-9">
                ...
                <div className="w-full mt-10 flex flex-col gap-9">
                    ...
                    <button className={(pedler()?.text != '' || günlükler()?.text != '' || tamponlar()?.text != '') ? "bg-footer-bg text-white font-medium text-lg w-full p-3 rounded-full cursor-pointer" : "bg-neutral-200 text-neutral-400 font-medium text-lg w-full p-3 rounded-full "} disabled={(pedler()?.text != '' || günlükler()?.text != '' || tamponlar()?.text != '') ? false : true}>Sepete Ekle (₺{totalPrice})</button>
                </div>
            </div>
        </div>
    )
